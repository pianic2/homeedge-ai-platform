...
| Blocker to EDA gate | Native schematic/PCB and ERC/DRC not yet captured/executed | KiCad 10.0.6 Flatpak CLI is available; controlled Eeschema launch produced no window, CLI cannot author schematics, and exact ESP32-C3-MINI-1 symbol/footprint must be created from Espressif pin/land data. No false ERC/DRC PASS. |
