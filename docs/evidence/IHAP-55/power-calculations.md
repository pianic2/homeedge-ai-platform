...
| MP2636 boost limit sensitivity | `ROLIM=220.8 kΩ` / 20 mΩ sense | 0.5 A nominal | **Rejected sensitivity only**; incompatible with the accepted >=0.5 A continuous and >=1.0 A transient capability target |
| MP2636 boost-limit candidates | `ROLIM=110.4 kΩ` or `90.3 kΩ` / 20 mΩ sense | 1.000 A or 1.223 A nominal by equation | candidates only; MPS table rounds 90.3 kΩ to 1.25 A; tolerance, battery current, thermal and transient margin pending |
...
The TMUX1511/TPS3839 gate remains `[UNVALIDATED]` for ADC input safety during `SYS_3V3` collapse: TPS3839's 20 µs falling propagation is typical, not a maximum. A 24 µs sensitivity interval at 0.5 A on the candidate 22 µF nominal 3.3 V output capacitor gives `ΔV = I·t/C ≈ 0.545 V`, exceeding the `2.857 − (2.625 − 0.3) = 0.532 V` ADC safety headroom at the minimum supervisor threshold even before capacitor DC-bias derating. Converter response and additional rail capacitance are unbounded; no supply-off safety PASS follows from this candidate.
