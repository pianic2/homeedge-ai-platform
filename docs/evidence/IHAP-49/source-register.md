...
`ihap-56-closure-matrix.md` records which requirements are Accepted versus Proposed.
...
- ADR-0007 — **Accepted baseline**: regulated 5 V USB-C normal source, LG INR18650-MJ1 backup-only cell, custom modular PCB direction, MP2636GR-P preferred charger/power-path/battery-boost candidate plus regulated 5 V post-stage or reviewed equivalent, 0.5 A continuous capability across accepted battery/valid USB ranges, reverse-insertion prevention and source-transfer/headroom targets. **Proposed IHAP-56 amendment**: source-side interruption/upstream-segment treatment, mandatory NTC fault-state tests, numeric thermal/low-voltage criteria, quantified backfeed, component-derived 3.3 V criteria, V13/V14/V15, strengthened V7/V8/V9 and ADR-0003 ownership extension.
...
The proposed first-reference product cutoff **2.70 ±0.05 V** and recovery **>=3.00 ±0.05 V** are project-level conservative treatment values above the manufacturer 2.50 V discharge-end value. They are neither LG requirements nor accepted project criteria until the Project Owner approves the amendment/treatment scope. The Proposed low V8/V9 transfer point is deliberately separated from cutoff: nominal **2.90±0.05 V** plus at least **100 mV loaded BATT margin above the maximum permitted cutoff** before USB removal.
...
**Thermal evidence limitation:** a package/case/board temperature is not automatically equal to junction temperature. Proposed RT-R012-01/RT-R013-01 thermal verification therefore requires IHAP-55 to justify a manufacturer-supported junction estimate or derive a conservative case/board ceiling from measured/calculated dissipation, datasheet thermal parameters applicable to the final PCB/layout, ambient and uncertainty. If that relation cannot be justified, the proposed MP2636 Tj criterion remains `[UNVALIDATED]`.

### ESP32-C3

Checked: **2026-09-09**.

- Espressif ESP32-C3 Series Datasheet — chip supply domain and Wi-Fi current envelope.
- The proposed R-013 3.3 V rail verification uses **3.0–3.6 V** as the initial ESP32-C3 outer supply bound. The final board PASS band is the intersection of all populated 3.3 V-load manufacturer ranges; any tighter OLED/environmental/interface-device requirement supersedes the ESP32-only outer bound.
- The current SuperMini-compatible development-board regulator capability remains `[UNVALIDATED]` and is not reused as the custom-board regulator specification.
...
Physical custom-board charging, regulated-rail, thermal, source-limit, dynamic load-step, source-transfer, backfeed and endurance claims remain `[UNVALIDATED]` until the applicable approved scope is implemented and tested.
