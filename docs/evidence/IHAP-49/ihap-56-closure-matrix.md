...
| Functional USB-C-to-USB-C operation through both DUT plug orientations | **Accepted verification consequence** | ADR-0007 USB-C contract | IHAP-55 | V2 both DUT orientations | Derives from accepted reversible Type-C contract; not a new treatment |
...
| High/mid/valid-low V8/V9 coverage + zero restoration-reset criterion | **Proposed strengthening** | R-013 / RT-R013-01 | IHAP-55 | Proposed V8/V9 strengthening | Requires explicit PO treatment/amendment approval |
...
1. `Accepted` means only the durable 2026-09-07 ADR-0007 / PR #34 decision evidence, plus direct verification consequences already inherent in that accepted interface contract (for example both reversible Type-C orientations).
...
7. No `no backfeed` treatment-effectiveness PASS is qualitative: Proposed strengthening defines measurable connector current/voltage limits.
8. 3.3 V treatment verification uses the intersection of populated-component supply limits; ESP32-C3 3.0–3.6 V is the initial outer bound, with tighter downstream limits taking precedence.
...
| Finding | Required closure | Closure state after 2026-09-09 sweep |
|---|---|---|
| Upstream holder faults | Proposed protection must be source-side of all in-scope wiring; otherwise upstream segment is explicitly uncovered and needs a separate control/verification before treatment can be approved/verified. | **Addressed in R-012, validation plan, power contract, power tree, owned-hardware evidence and downstream contract** |
| Reverse insertion while USB present | Proposed V15 split into USB-absent and USB-present bounded simulator cases. | **Addressed as V15-A/V15-B** |
| MP2636 junction limit | Proposed thermal method requires a justified junction estimate or conservative case/board derating derived from datasheet thermal parameters, loss, layout and ambient. | **Addressed in validation plan, R-013, source register, risk summary and power contract** |
| 0.5 A accepted range | Restore accepted range qualifiers and exercise relevant battery/USB endpoints in V6. | **Addressed in ADR, V6, downstream contract, power tree, indexes and risk surfaces** |
| Low transfer point vs cutoff | Proposed low V8/V9 condition must maintain margin above the maximum allowed cutoff under load. | **Old 2.80±0.05 V retired; nominal 2.90±0.05 V plus >=100 mV loaded margin** |
| Backfeed criterion | Proposed V8/V9 defines USB VBUS and upstream-current limits in disconnected and attached-unpowered-source conditions. | **<=0.30 V open VBUS and <=1.0 mA attached-unpowered-source current unless tighter limits apply** |
| 3.3 V rail | Proposed overlay defines component-derived steady/transient limits in all relevant power tests. | **Initial ESP32-C3 3.0–3.6 V outer bound; tighter populated part wins** |
| USB-C orientations | Accepted V2 functionally tests C-to-C operation with both DUT plug orientations. | **Addressed in V2 and downstream/evidence routing** |
