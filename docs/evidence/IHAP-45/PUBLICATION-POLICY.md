...
2. `docs/evidence/IHAP-45/.gitignore` blocks common raw-evidence filenames and unsanitized HEIC files.
3. The summary publisher reads local runs but writes new files from an allow-list of aggregate fields.
4. The publisher does not copy source files and does not embed raw JSONL lines.
5. Validation errors and warnings are preserved in published summaries.
6. A failed run may be summarized for transparency but cannot be presented as accepted evidence.
7. Raw evidence is not required to remain indefinitely; the Project Owner controls local retention.
...
The generated environmental summary may include:
...
Before committing generated summaries, the Project Owner must inspect both Markdown and JSON outputs, confirm that the status is accurate and verify with `git diff --cached` that no raw files are staged.
